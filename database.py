...
class Elastic_Search(object):
    ...
    def write_large_csv(self, file_path, index_name=None, chunksize=10000, embedder=None, field_to_embed=None, index_field=None):
        ...
        for chunk in tqdm.tqdm(df_chunk):
            if embedder:
                for i in field_to_embed:
                    chunk[f'{i}_embedding'] = embedder(chunk[i].values)
    ...
    def sample(self, index_name=None, size=3):
        ...
        return res
    def span_search_query(self,search_query):
        ...
        for w in words:
            nest = nested_dict()
            nest["span_term"]["title"]=w
            # nest["span_multi"]["match"]["fuzzy"]["title"]["fuzziness"]="2"
            json.dumps(nest)
    ...
    def search(self, query, field, type='match', index_name=None, embedder=None, size=10):
        ...
        if type=='dense':
           ...
           # Here field is a list not str
        elif type == 'multi_match':
            res = self.es.search(index=index_name,body={'query': {"multi_match": {"fields": field,"query": query, "fuzziness": "AUTO"}},"_source": {"excludes": [f'{field[0]}_embedding', f'{field[1]}_embedding']}}, size=size)
    
        else:
            res=self.es.search(index=index_name, body={'query':{type:{field:{"query":query,"fuzziness": "AUTO"}}}, "_source": {"excludes": [f'{field}_embedding']}},size=size)
